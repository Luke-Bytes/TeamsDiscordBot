...
export default class CaptainCommand implements Command {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!member || !member.roles) {
      await interaction.reply({ content: "Couldn't find role data - do I need more permissions?.", ephemeral: true });
      return;
    }
    ...
    if (!player) {
      await interaction.reply({ content: "This player hasn't registered!", ephemeral: true });
      return;
    }

    const inGameName = player.getInGameName();
    if (!GameData.getPlayers().includes(inGameName)) {
      await interaction.reply({ content: `${inGameName} is not in the current game players list. Captain cannot be set.`, ephemeral: true });
      return;
    }

    const previousCaptain = this.getCaptainByTeam(teamColor);
    if (previousCaptain) {
      console.log(`Previous captain found: ${previousCaptain.getDiscordUserName()}`);

      const previousMember = interaction.guild.members.cache.get(previousCaptain.getDiscordUserId());
      if (previousMember) {
        console.log(`Removing captain role from: ${previousMember.user.username}`);
        previousCaptain.setCaptain(false); // Set the isCaptain flag to false
        await previousMember.roles.remove(captainRoleId)
          .then(() => console.log(`Captain role removed from: ${previousMember.user.username}`))
          .catch((error) => console.error(`Failed to remove captain role: ${error}`));
      } else {
        console.log(`Previous captain member not found in guild: ${previousCaptain.getDiscordUserName()}`);
      }
    } else {
      console.log(`No previous captain found for ${teamColor} team, proceeding..`);
    }
    ...
    if (newCaptainMember) {
      await newCaptainMember.roles.add(captainRoleId)
        .then(() => console.log(`Captain role added to: ${newCaptainMember.user.username}`))
        .catch((error) => console.error(`Failed to add captain role: ${error}`));
      ...
      await interaction.reply({ content: `${user.displayName} has been set as the ${teamColor} team captain.` });
    } else {
      console.log(`Failed to fetch member for new captain: ${user.displayName}`);
      await interaction.reply({ content: 'Could not assign captain role. Is this name correct?', ephemeral: true });
    }
  }

  private getCaptainByTeam(teamColor: 'blue' | 'red'): PlayerData | null {
    const captain = PlayerData.playerDataList.find(player => player.getIsCaptain() && this.isPlayerOnTeam(player, teamColor));

    if (captain) {
      console.log(`Captain for ${teamColor} team is: ${captain.getDiscordUserName()}`);
    } else {
  ...
  private isPlayerOnTeam(player: PlayerData, teamColor: 'blue' | 'red'): boolean {
    const inGameName = player.getInGameName();
    if (teamColor === 'blue') {
