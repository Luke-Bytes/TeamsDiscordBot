...
export default class MVPCommand implements Command {
  data = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("vote")
        .setDescription("Type 'vote' to cast an MVP vote.")
        .setRequired(true)
        .addChoices({ name: "vote", value: "vote" })
    )
    .addStringOption((option) =>
      option
        .setName("player")
        .setDescription("The username or Discord ID of the player to vote for")
        .setRequired(true)
    );

  name = "mvp";
  description = "Vote for the MVP in your team.";
  buttonIds = [];

  async execute(interaction: ChatInputCommandInteraction) {
    const action = interaction.options.getString("vote", true);
