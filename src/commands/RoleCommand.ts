...
export default class CaptainCommand implements Command {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!member || !member.roles) {
      await interaction.reply({
        content: "Could not retrieve your guild member data.",
        ephemeral: true,
      });
      return;
    }
    ...
    const teamRoleId =
      teamColor === "blue"
        ? configData.roles.blueTeamRole
        : configData.roles.redTeamRole;

    const previousCaptain = this.getCaptainByTeam(teamColor as "blue" | "red", interaction);
    if (previousCaptain) {
    ...
    if (newCaptainMember) {
      ...
      await interaction.reply({
        content: `${user.username} has been set as the ${teamColor} team captain.`,
      });
    } else {
      await interaction.reply({
        content:
          "Could not assign captain role. The user might not be a guild member.",
        ephemeral: true,
      });
    }
  }
      ...
      PlayerData.playerDataList.find(
        (player) =>
          ...
          interaction.guild?.members.cache
            .get(player.getDiscordUserId())
            ?.roles.cache.has(teamRoleId)
      ) || null
    );
  }
}
