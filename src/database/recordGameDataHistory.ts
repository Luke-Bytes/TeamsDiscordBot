...
async function recordGameDataHistory(gameData: GameData) {
    ...
    if (!winningTeam) {
        console.error("Game winner hasn't been set yet, cannot save game history until game has ended.");
        return;
    }
    ...
    const minerushing = minerushingVotes.length > 0 && minerushingVotes[0] === 'yes';
