...
test("personal season wrapped generates a compact player recap", () => {
  ...
  assert(text.includes("4W-1L"), "Includes record");
  assert(text.includes("1130 final"), "Includes final Elo");
  assert(text.includes("Collected 3 MVPs"), "Includes personalised highlight");
});
