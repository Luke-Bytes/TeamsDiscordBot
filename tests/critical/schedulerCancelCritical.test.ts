...
const fastTest = (name: string, fn: () => Promise<void> | void) =>
  test(name, () => withImmediateTimers(fn));

fastTest(
  "resetCurrentGame cancels scheduled map/minerush vote tasks",
  async () => {
    ...
    assert(
      mapFired === 0,
      `expected mapVote task not to fire (got ${mapFired})`
    );
    assert(
      minerushFired === 0,
